...
--
-- [ from https://git.zx2c4.com/cgit/tree/filters/simple-authentication.lua ]
--
...
function authenticate_post()
	...
	if not (post["username"] == auth_user or post["password"] == auth_pass) then
		set_cookie("cgitauth", "")
	else
		-- One week expiration time
		local username = secure_value("username", post["username"], os.time() + auth_expire)
...
function authenticate_cookie()
	...
	if username == nil or not username == auth_user then
		return 0
	else
		return 1
	end
